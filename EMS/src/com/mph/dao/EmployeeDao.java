...
public class EmployeeDao
{
	...
	public void viewEmp()
	{
		...
		public void type_scroll_insensitive_rs()
		{
			con = MyDBConnection.getDBConnection();
			try {
				ps = con.prepareStatement("select eno,ename from mphemp",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
				rs = ps.executeQuery();
		...
		public void type_scroll_sensitive_rs_insert()
		{
			con = MyDBConnection.getDBConnection();
			try {
				ps = con.prepareStatement("select eno,ename from mphemp where eno=3",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
			//	ps = con.prepareStatement("update mphemp set eno=11 where eno=2",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
				rs = ps.executeQuery();
		...
		public void batchUpdate()
		{
			con = MyDBConnection.getDBConnection();
			 try {
				st= con.createStatement();
				st.addBatch("update mphemp set ename = 'Meena' where eno=10");
				st.addBatch("update mphemp set eno=13 where ename='sowmya'");
				st.addBatch("update mphemp set ename = 'vitika' where eno=3");
				st.addBatch("update mphemp set ename = 'sarika' where eno=10");
				st.addBatch("update mphemp set ename = 'Mohan' where eno=9");
				st.addBatch("update mphemp set ename = 'Dhana' where eno=8");
